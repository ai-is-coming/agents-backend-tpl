...
export const AgentChatRequestSchema = z
  .object({
    ...
    provider: z.string().default('deepseek').openapi({ description: 'LLM provider', example: 'deepseek' }),
    model: z.string().optional().openapi({ description: 'Model id (provider-specific)', example: 'deepseek-chat' }),
  })
